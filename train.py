...
def training():
    ...
    predictions, softmax, train_logits, p = inference_op(image_train_batch, keep_prob)
    ...
    try:
        for step in range(MAX_STEP):
            ...
            _, loss, acc = sess.run([train_op, train_loss, train_acc], {keep_prob: 0.75})
...
def eval():
    ...
    try:
        for step in range(MAX_STEP):
            ...
            # image, prediction = sess.run([image_train_batch, train_logits], {keep_prob: 1.0})
            max_index = np.argmax(prediction)
            if max_index == 0:
                label = '%.2f%% is a cat.' % (prediction[0][0] * 100)
                mean = mean + prediction[0][0] * 100
            else:
                label = '%.2f%% is a dog.' % (prediction[0][1] * 100)
                mean = mean + prediction[0][1] * 100

            plt.imshow(image[0])
            plt.title(label)
            plt.show()
        print("Average accuracy：%.2f%% " % (mean / MAX_STEP))
    except tf.errors.OutOfRangeError:
        print('Done.')
    finally:
        coord.request_stop()
